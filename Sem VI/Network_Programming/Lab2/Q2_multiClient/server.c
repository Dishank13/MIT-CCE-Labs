...
int client_counter = 0; // Global counter for client IDs
    ...
    while(1){
        ...
        if (pid == 0) {
            ...
            while(1){
                ...
                printf("\n Server to Client #%d: ", current_client_id);
            ...
            printf("\n Connection Closing for Client #%d\n", current_client_id);
            close(newsockfd);
            exit(0);
        }
        else {
            /* PARENT PROCESS */
            close(newsockfd); // parent continues accepting new clients
        }
    }
